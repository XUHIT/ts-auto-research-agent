...
from ts_auto_research.report import generate_report
...
class CoreLoopTests(unittest.TestCase):
    ...
    def test_smoke_loop_creates_two_runs_and_protocol_files(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            for run_number in ["run_0001", "run_0002"]:
                ...
                self.assertTrue((run_dir / "experiment_schema.json").exists())
                self.assertTrue((run_dir / "schema_validation.json").exists())
                self.assertTrue((run_dir / "protocol_audit.md").exists())
    ...
    def test_multiagent_orchestration_writes_recoverable_trace(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertIn("code_engineer", agent_ids)
            self.assertIn("experiment_runner", agent_ids)
            self.assertIn("synthesis_agent", agent_ids)
            self.assertIn("reporter_agent", agent_ids)
            lane_names = {lane["display_name"] for lane in trace["role_lanes"]}
            self.assertEqual(lane_names, {"Planner", "Engineer", "Executor", "Evaluator", "Reporter"})
    ...
    def test_full_research_cli_uses_server_defaults(self) -> None:
        with TemporaryDirectory() as tmp:
            ...
            self.assertEqual(kwargs["literature_limit"], 1000)
            self.assertEqual(kwargs["train_epochs"], 3)


    def test_report_generation_creates_visual_artifacts(self) -> None:
        with TemporaryDirectory() as tmp:
            workspace = self.workspace(tmp)
            init_workspace(workspace)
            leaderboard_text = (
            ...
            metrics_by_run = {
                "run_0001": {"model": "DLinear", "role": "baseline_anchor", "value": 0.59827763, "mae": 0.38131171, "delta": 0.0},
                "run_0002": {"model": "PatchTST", "role": "strong_reference", "value": 0.58627319, "mae": 0.37807289, "delta": 0.01200444},
                "run_0003": {"model": "CalDLinear", "role": "innovation_candidate", "value": 0.59605795, "mae": 0.38774657, "delta": 0.00221968},
            }
                    ...
                    json.dumps(
                        {
                            ...
                            "baseline": 0.59827763,
                            "delta": item["delta"],
                            "diagnostics": {"model": item["model"], "method_role": item["role"], "mae": item["mae"]},
                        }
                    ),
                    encoding="utf-8",
                )
            artifacts = generate_report(workspace, output_dir=Path(tmp) / "report")
            dashboard = Path(artifacts["dashboard_html"])
            monitor = Path(artifacts["monitor_html"])
            pdf = Path(artifacts["pdf_report"])
            cockpit = Path(artifacts["research_cockpit_html"])
            demo_packet = Path(artifacts["demo_packet_json"])
            self.assertIn("CalDLinear improves", dashboard.read_text(encoding="utf-8"))
            self.assertIn("refresh", monitor.read_text(encoding="utf-8"))
            self.assertIn("Single-Benchmark Research Cockpit", cockpit.read_text(encoding="utf-8"))
            self.assertEqual(json.loads(demo_packet.read_text(encoding="utf-8"))["status"], "generated")
            self.assertEqual(pdf.read_bytes()[:8], b"%PDF-1.4")
            self.assertTrue(Path(artifacts["metrics_svg"]).exists())
            self.assertTrue(Path(artifacts["delta_svg"]).exists())
